export const HIGHLIGHTED_EVENT_ITEMS = [
  {
    id: 7,
    slug: 'dinner-awards-2026',
    registrationLink: 'https://forms.gle/SmuFyV1gNv23oy1BA',
    time: '7:00 PM',
    timestamp: '2026-09-01T19:00:00',
    venue: 'To Be Disclosed',
    details: {
      title: 'ULES Dinner & Awards 2026',
      description:
        "VIVA LAS VEGAS, A Casino Experience: ULES Dinner & Awards 2025/26.\nWhat's a better combo than an Award Gala with a Casino Experience?\nDon't miss the Faculty of Engineering's Dinner Awards 2026, themed \"VIVA LAS VEGAS\"\nCome in your casino themed outfits and experience Las Vegas in Lagos.🧏🏾‍♂️",
      date: 'September 1st, 2026',
      category: 'Annual Event',
      shortAbout:
        'VIVA LAS VEGAS, A Casino Experience: ULES Dinner & Awards 2025/26.',
      learnMoreHref: '/activities/events/dinner-awards-2026',
    },
    imageUrl: '/events/dinner-awards-2026.png',
    imageAlt: 'ULES Dinner & Awards 2026',
  },
  {
    id: 6,
    slug: 'pidec-2026',
    registrationLink: 'https://www.pidec.com.ng/finale',
    time: '9:00 AM',
    timestamp: '2026-08-28T12:00:00',
    venue: 'Prof. J.F. Ajayi Hall, UNILAG',
    details: {
      title: 'PIDEC 1.0 Grand Finale',
      description:
        "The battle to be called the PIDEC 1.0 champion will come to a conclusion on the 28th August, 2026. Dignitaries, Important People in the Engineering World and seeing departments come up with brilliant innovations, PIDEC 1.0 is looking exciting and we can't wait to see you there!",
      date: 'August 28th, 2026',
      category: 'Annual Event',
      shortAbout:
        'The battle to be called the PIDEC 1.0 champion comes to a conclusion on the 28th of August, 2026.',
      learnMoreHref: '/activities/events/pidec-2026',
    },
    imageUrl: '/events/pidec-2026.jpeg',
    imageAlt: 'PIDEC 1.0 Grand Finale',
  },
  {
    id: 5,
    slug: 'student-symposium-2026',
    registrationLink: 'https://luma.com/rdf2fvwi',
    time: '9:00 AM',
    timestamp: '2026-08-28T12:00:00',
    venue: 'FSS Design Studio, UNILAG',
    details: {
      title: 'Student\'s Symposium 3.0',
      description:
        "You've heard about the red carpet interviews👗\nYou've heard about the Awards Night 🏅\n5 Communities.\n5 panelists.\n1 Mechanical Engineering\n\nNIMechE UNILAG SF brings you the third edition of the Students´ Symposium, don't miss the conversation that defines our generation: MECHANICAL ENGINEERING IN THE ERA OF AI.",
      date: 'September 1st, 2026',
      category: 'Annual Event',
      shortAbout:
        'NIMechE UNILAG SF brings you the third edition of the Students\' Symposium: Mechanical Engineering in the Era of AI.',
      learnMoreHref: '/activities/events/student-symposium-2026',
    },
    imageUrl: '/events/student-symposium-2026.jpeg',
    imageAlt: 'Student\'s Symposium 3.0',
  },
  {
    id: 4,
    slug: 'sees-lightup-conference-2026',
    registrationLink: 'https://bit.ly/SEESLightUpConference',
    time: '9:00 AM',
    timestamp: '2026-08-27T12:00:00',
    venue: 'Jelili Omotola Multipurpose Hall, UNILAG',
    details: {
      title: 'SEES LIGHT UP 2.0',
      description:
      "Ideas are cheap. Impact is the hard part. 💚⚡\nSEES LIGHT UP CONFERENCE 2.0 is all about Igniting Innovation, bridging Ideas to Impact.\nIt is completely FREE and open to every student.",
      date: 'August 27th, 2026',
      category: 'Annual Event',
      shortAbout:
        'Igniting innovation, bridging ideas to impact. Completely FREE and open to every student.',
      learnMoreHref: '/activities/events/sees-lightup-conference-2026',
    },
    imageUrl: '/events/lightup-conference-2026.jpeg',
    imageAlt: 'SEES LIGHT UP CONFERENCE 2.0',
  },
  {
    id: 3,
    slug: 'systems-synergy-2026',
    registrationLink: null,
    time: '9:00 AM',
    timestamp: '2026-08-27T12:00:00',
    venue: 'To Be Disclosed',
    details: {
      title: 'SYSTEMS SYNERGY 2.0',
      description:
      "ARE YOU READY FOR THE WHOLE PICTURE ⁉️\nAccording to Beyoncé, the principle of synergy states that the WHOLE system is always greater than the sum of its individual parts 🔄\nOn August 27th, everything we’ve been building comes together 🚀\nGet ready for SYSTEMS SYNERGY 2.0",
      date: 'August 27th, 2026',
      category: 'Annual Event',
      shortAbout:
        'On August 27th, everything we\'ve been building comes together. Get ready for SYSTEMS SYNERGY 2.0.',
      learnMoreHref: '/activities/events/systems-synergy-2026',
    },
    imageUrl: '/events/systems-synergy-2026.jpeg',
    imageAlt: 'SYSTEMS SYNERGY 2.0',
  },
  {
    id: 2,
    slug: 'usf-2026',
    registrationLink: null,
    time: '9:00 AM',
    timestamp: '2026-06-24T09:00:00',
    venue: 'ULES Sports Complex',
    details: {
      title: 'ULES Sports Festival 2026',
      description:
        'The ULES Sports Festival is an annual Sports Fest which sees 9 departments slug it out in different sports and games to find out who will be the champion of ULES.',
      date: 'June 21st-24th, 2026',
      category: 'Annual Event',
      shortAbout:
        'An annual sports fest where 9 departments slug it out across different sports and games to find the champion of ULES.',
      learnMoreHref: '/activities/events/usf-2026',
    },
    imageUrl: '/events/usf-2026.png',
    imageAlt: 'ULES Sports Festival 2026',
  },
  {
    id: 1,
    slug: 'quadtopia-2025',
    registrationLink: null,
    time: '4:00 PM',
    timestamp: '2025-12-17T16:00:00',
    venue: 'Engineering Quadrangle',
    details: {
      title: 'QUADTOPIA',
      description:
        "Social Night this year isn't just a party, it's an experience.\n With an all-white dress code, the night transforms into a Circus Playground filled with music, games, glow vibes, doodles, food and memories.",
      date: 'December 17th, 2025',
      category: 'Annual Event',
      shortAbout:
        'Social Night this year isn\'t just a party, it\'s an experience — an all-white Circus Playground.',
      learnMoreHref: '/activities/events/quadtopia-2025',
    },
    imageUrl: '/events/quadtopia-2025.png',
    imageAlt: 'QUADTOPIA by ULES',
  },
  {
    id: 0,
    slug: 'fresher-orientation-2025',
    registrationLink: null,
    time: '10:00 AM',
    timestamp: '2025-12-16T10:00:00',
    venue: 'Faculty of Engineering Auditorium',
    details: {
      title: "ULES Fresher's Orientation 2025",
      description:
        'Join us for an exciting orientation packed with insights, games, and connections to set you up for success in UNILAG Engineering.',
      date: 'December 16th, 2025',
      category: 'Annual Event',
      shortAbout:
        'An orientation packed with insights, games, and connections to set you up for success in UNILAG Engineering.',
      learnMoreHref: '/activities/events/fresher-orientation-2025',
    },
    imageUrl: '/events/fresher-orientation-2025.png',
    imageAlt: "ULES Fresher's Orientation for 2025",
  },
];
